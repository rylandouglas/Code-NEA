...
def show_calendar_frame():
    ...
    # Create frame 4 to place calendar on
    ...
    calendar_label = customtkinter.CTkLabel(frame4, text="Book a New Job", font=font1, text_color="white")
    calendar_label.place(x=150, y=20)
    ...
    selected_date_label.place(x=160, y=280)

    # Function to confirm the booking
    def confirm_booking():
        selected_date = calendar.get_date()
        # You can replace this with actual booking logic, for example saving to the database
    ...
    confirm_button.place(x=180, y=320)

    # Function to handle date selection
    def on_date_selected(event):
        selected_date = calendar.get_date()
        selected_date_label.configure(text=f"Selected Date: {selected_date}")
        confirm_button.configure(state="normal")  # Enable the confirm button when a date is selected
    ...
    close_button.place(x=1775, y=20)
